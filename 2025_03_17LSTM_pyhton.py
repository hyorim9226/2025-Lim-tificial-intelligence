...
class LSTM:
    ...
    def __init__(self, n_h, n_x):
        ...
        self.cache = None
        self.param = [self.W_f, self.W_u, self.W_o, self.W_c, self.b_f, self.b_u, self.b_o, self.b_c, self.W_y, self.W_y]
    ...
    def update(self, learning_rate):
        ...
        self.W_y -= learning_rate * self.grads[-2]
        self.b_y -= learning_rate * self.grads[-1]
...
for i in range(epoch):
    for j in range(len(x)):
        h_t , c_t, y_hat, z_t = lstm.forward(h_t, x[j].reshape(-1, 1), c_t)
        Y.append([y[j], y_hat[0][0]])
#        print(h_t.T, c_t.T)
    print(z_t)
    ...
    for k in range(len(x)-1, 0, -1):
        dh_next, dc_next = lstm.backward(dh_next, dc_next, y[k])

    lstm.update(learning_rate = learning_rate)    
# %%
print(Y)
