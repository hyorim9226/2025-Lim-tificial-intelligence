...
class LSTM:
    ...
    def __init__(self, n_h, n_x):
        self.n_h = n_h
        self.n_x = n_x
        self.W_f = np.random.randn(n_h, n_x + n_h) * np.sqrt(2/(n_h+n_x))
        self.W_u = np.random.randn(n_h, n_x + n_h) * np.sqrt(2/(n_h+n_x))
        self.W_o = np.random.randn(n_h, n_x + n_h) * np.sqrt(2/(n_h+n_x))
        self.W_c = np.random.randn(n_h, n_x + n_h) * np.sqrt(2/(n_h+n_x))
        
        self.b_f = np.random.randn(n_h, 1) * 0.01
        self.b_u = np.random.randn(n_h, 1) * 0.01
        self.b_o = np.random.randn(n_h, 1) * 0.01
        self.b_c = np.random.randn(n_h, 1) * 0.01

        # 출력층
        self.W_y = np.random.randn(1, n_h) * 0.01
        self.b_y = np.zeros((1, 1))

        # 캐시 저장..? 역전파에 필요하대서 씁니다
        self.cache = None
        self.grads = [np.zeros_like(self.W_f), np.zeros_like(self.W_u), np.zeros_like(self.W_o), np.zeros_like(self.W_c), np.zeros_like(self.b_f), np.zeros_like(self.b_u), np.zeros_like(self.b_o), np.zeros_like(self.b_c)]
    ...
    def forward(self, h_past, x_t, c_past):
        ...
        c_t = f_t * c_past + u_t * headc_t # 이게 c_next로 cache에 저장
        h_t = o_t * self.tanh(c_t) # 이것도 h_next로 cache에 저장
    ...
    def backward(self, dh_next, dc_next):
        x_t, h_past, c_past, f_t, u_t, o_t, headc_t, c_next = self.cache
        ...
        dh_past = np.dot(self.W_f[:, :n_h].T, df) + np.dot(self.W_u[:, :n_h].T, du) + np.dot(self.W_o[:, :n_h].T, do) + np.dot(self.W_c[:, :n_h].T, dc)
        # dx = np.dot(self.W_f[:, n_h:].T, df) + np.dot(self.W_u[:, n_h:].T, du) + np.dot(self.W_o[:, n_h:].T, do) + np.dot(self.W_c[:, n_h:].T, dc)
...
n_x = 5
n_h = 10 
lstm = LSTM(n_x = n_x, n_h = n_h)

# %%
def update(self, learning_rate):
    for i, grad in enumerate(self.grads):
        self.param[i] -= learning_rate * grad
# %%
...
epoch = 10
h_t = np.random.randn(n_h, 1)
c_t = np.random.randn(n_h, 1)
learning_rate = 0.001 #이거 알아서 설정해야하는것같은데 우예하는지 모르겟노
y = np.array([1 for i in range(20)] + [0 for _ in range(20)])
Y = [] # loss가 이미 있는 거라ㅅ..   [yi, hat_y]
for i in range(epoch):
    for j in range(len(x)):
        h_t , c_t, y_hat, z_t = lstm.forward(h_t, x[i].reshape(-1, 1), c_t)
        Y.append([y[j], y_hat])
    loss = np.log(Y[-1][1])*Y[-1][0]*(-1)
    print(loss)
    for k in range(len(x)-1, 0, -1):
        h_t, c_t = lstm.backward(h_t, c_t)
# %%
print(h_t, c_t, loss)
# %%
